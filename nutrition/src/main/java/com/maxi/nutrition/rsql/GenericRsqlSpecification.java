package com.maxi.nutrition.rsql;
  ...
  public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query,
    ...
    switch (RsqlSearchOperation.getSimpleOperator(operator)) {
      case EQUAL: {
        if (argument instanceof String) {
          ...
          return builder.equal(
              root.<LocalDateTime>get(property), LocalDateTime.parse(argument.toString()));
        } else {
      ...
      case GREATER_THAN_OR_EQUAL: {
        if (argument instanceof LocalDateTime) {
          return builder.greaterThanOrEqualTo(
              root.<LocalDateTime>get(property), (LocalDateTime) argument);
        } else {
          return builder.greaterThanOrEqualTo(
      ...
      case LESS_THAN_OR_EQUAL: {
        if (argument instanceof LocalDateTime) {
          return builder
              .lessThanOrEqualTo(root.<LocalDateTime>get(property), (LocalDateTime) argument);
        } else {
    ...
    for (String argument : arguments) {
      if (type.equals(Integer.class)) {
        ...
        if (argument.length() == 10) {
          args.add(LocalDate.parse(argument).atStartOfDay());
        } else {
